...
(() => {
  ...
  function renderDashboard(root) {
    ...
    root.innerHTML = `
      ...
      <div class="panel">
        <div class="panel-title-row"><h2 class="panel-title">→ Next actions</h2></div>
        ${nextActions.length ? nextActions.slice(0,8).map(p=>`
          <div class="project-card" data-open-project="${esc(p.id)}">
            ...
            <div class="row-meta"><span>${esc(p.name)}</span><span>${esc(p.priority)} priority</span></div>
        ...
        <div class="panel">
          ...
          ${upcomingMeetings.length?upcomingMeetings.map(m=>`
            <div class="project-card" data-open-meeting="${esc(m.id)}">
              ...
              <div class="row-meta"><span>${dateOnly(m.date)}</span><span>${esc(projectById(m.projectId)?.name||"General")}</span></div>
    ...
    root.innerHTML=`
      <div class="panel">
        <div class="form-grid">
          ...
          <div><label class="field-label">Next action</label><div>${esc(p.nextAction||"Not set")}</div></div>
      ...
      <div class="dashboard-two">
        <div class="panel">
          <div class="panel-title-row"><h2 class="panel-title">→ Next action</h2></div>
          <div class="big-next">${esc(p.nextAction||"No next action set")}</div>
        </div>
        <div class="panel">
  ...
  function renderProjectGantt(root,p){
    ...
    $("#addMilestoneBtn").onclick=()=>openMilestoneModal(p);
    $$("[data-edit-milestone]",root).forEach(b=>b.onclick=()=>openMilestoneModal(p,p.milestones.find(m=>m.id===b.dataset.editMilestone)));
  }
  ...
  function renderProjectTasks(root,p){
    ...
    $("#manageSectionsBtn").onclick=()=>openSectionsModal(p);
    $("#tabAddTaskBtn").onclick=()=>openTaskModal(p);
    bindProjectTaskRows(root,p);
  }

  function bindProjectTaskRows(root,p){
    $$(".task-check",root).forEach(ch=>ch.addEventListener("change",()=>{
      const t=p.tasks.find(x=>x.id===ch.dataset.taskId);if(!t)return;
      t.done=ch.checked;t.updatedAt=nowISO();p.updatedAt=nowISO();persist();renderProject(root.closest(".content")||$("#content"),p.id);
    }));
    $$("[data-edit-task]",root).forEach(b=>b.onclick=()=>openTaskModal(p,p.tasks.find(t=>t.id===b.dataset.editTask)));
    $$("[data-delete-task]",root).forEach(b=>b.onclick=()=>{
      const t=p.tasks.find(x=>x.id===b.dataset.deleteTask);
      if(t&&confirm(`Delete "${t.title}"?`)){p.tasks=p.tasks.filter(x=>x.id!==t.id);p.updatedAt=nowISO();persist();renderProject($("#content"),p.id);}
    });
  }

  function renderProjectMeetings(root,p){
    const meetings=state.meetings.filter(m=>m.projectId===p.id).sort((a,b)=>String(b.date).localeCompare(String(a.date)));
  ...
  function renderProjectNotes(root,p){
    ...
    $("#saveNotesBtn").onclick=()=>{p.notes=$("#projectNotes").value;p.updatedAt=nowISO();persist();toast("Notes saved");};
  ...
  function meetingCardHTML(m){
    ...
    return `<div class="meeting-card">
      <div class="meeting-top"><div><div class="row-title">${esc(m.title)}</div><div class="row-meta"><span>${dateOnly(m.date)}</span><span>${esc(p?.name||"General")}</span>${m.participants?`<span>${esc(m.participants)}</span>`:""}</div></div>
      <button class="row-button" data-edit-meeting="${esc(m.id)}">Open</button></div>
      ${m.summary?`<div class="meeting-summary">${esc(m.summary)}</div>`:""}
  ...
  function openMeetingModal(existing=null,presetProjectId=""){
    const m=existing||{title:"",date:new Date().toISOString().slice(0,10),projectId:presetProjectId,participants:"",summary:"",decisions:[],questions:[],actions:[]};
    const decisions=(m.decisions||[]).join("\n");
    const questions=(m.questions||[]).join("\n");
    const actions=(m.actions||[]).map(a=>a.text||a).join("\n");
    openModal(existing?"Meeting summary":"New meeting",`
      <div class="form-grid">
      ...
      <div class="form-row"><label class="field-label">Project</label><select class="field" id="mmProject"><option value="">General / no project</option>${state.projects.map(p=>`<option value="${esc(p.id)}" ${p.id===m.projectId?"selected":""}>${esc(p.name)}</option>`).join("")}</select></div>
      ...
      <div class="form-row"><label class="field-label">Summary</label><textarea class="field meeting-text" id="mmSummary">${esc(m.summary||"")}</textarea><button type="button" class="button secondary small" id="dictateMeetingBtn">🎙 Dictate summary</button></div>
      ...
      <div class="form-row"><label class="field-label">New action items — one per line</label><textarea class="field" id="mmActions" placeholder="Each line becomes a task">${existing?"":esc(actions)}</textarea></div>
    ...
    `,`<button type="button" class="button secondary" id="cancelModalBtn">Cancel</button><button type="button" class="button primary" id="saveMeetingBtn">Save meeting</button>`,()=>{
      ...
      $("#dictateMeetingBtn").onclick=()=>startDictation($("#mmSummary"),{append:true});
      $("#saveMeetingBtn").onclick=()=>{
        const title=$("#mmTitle").value.trim();if(!title){toast("Meeting title is required");return;}
        const projectId=$("#mmProject").value;
        ...
        for(const text of actionLines){
          if(projectId){
            ...
            const task={id:uid(),title:text,priority:"Normal",dueDate:"",tags:[],notes:`From meeting: ${title}`,waitingFor:"",done:false,createdAt:nowISO(),updatedAt:nowISO()};
            state.generalTasks.push(task);linkedActions.push({text,taskId:task.id,projectId:""});
          }
        }
        const data={title,date:$("#mmDate").value,projectId,participants:$("#mmParticipants").value.trim(),summary:$("#mmSummary").value.trim(),
        ...
        if(existing)Object.assign(existing,data);else state.meetings.unshift({id:uid(),createdAt:nowISO(),...data});
        persist();closeModal();render();toast("Meeting saved");
      };
      if(existing)$("#deleteMeetingBtn").onclick=()=>{if(confirm("Delete this meeting summary? Linked tasks will remain.")){state.meetings=state.meetings.filter(x=>x.id!==m.id);persist();closeModal();render();}};
    });
  }
  ...
  // ---------- Milestones ----------
  function openMilestoneModal(p,existing=null){
    ...
    `,`<button type="button" class="button secondary" id="cancelModalBtn">Cancel</button><button type="button" class="button primary" id="saveMilestoneBtn">Save</button>`,()=>{
      ...
      $("#saveMilestoneBtn").onclick=()=>{const title=$("#milTitle").value.trim();if(!title){toast("Milestone title is required");return;}
        ...
        if(existing)Object.assign(existing,data);else p.milestones.push({id:uid(),createdAt:nowISO(),...data});
        p.updatedAt=nowISO();persist();closeModal();currentProjectTab="gantt";renderProject($("#content"),p.id);};
      if(existing)$("#deleteMilestoneBtn").onclick=()=>{if(confirm("Delete this milestone?")){p.milestones=p.milestones.filter(x=>x.id!==m.id);p.updatedAt=nowISO();persist();closeModal();renderProject($("#content"),p.id);}};
    });
  }
  ...
  function renderSearchResults(root,q) {
    ...
    const ms=state.meetings.filter(m=>[m.title,m.participants,m.summary,(m.decisions||[]).join(" "),(m.questions||[]).join(" ")].join(" ").toLowerCase().includes(q));
    root.innerHTML=`
  ...
  function renderSettings(root) {
    root.innerHTML=`
      <div class="page-header"><div><h1 class="page-title">Settings & Sync</h1><div class="row-meta">ResearchFlow v0.4.1</div><p class="page-subtitle">Local-first storage with optional OneDrive synchronization</p></div></div>
    ...
    `,`<button class="button secondary" type="button" id="cancelModalBtn">Cancel</button><button class="button primary" type="button" id="saveProjectBtn">${existing?"Save":"Create"}</button>`,()=>{
      ...
      $("#saveProjectBtn").onclick=()=>{
        ...
        if(existing){
          ...
          const np={id:uid(),name,summary:$("#mpSummary").value.trim(),status:$("#mpStatus").value,priority:$("#mpPriority").value,nextAction:$("#mpNext").value.trim(),deadline:$("#mpDeadline").value,tags:normalizeTags($("#mpTags").value),notes:"",sections:[...DEFAULT_SECTIONS],tasks:[],milestones:[],createdAt:nowISO(),updatedAt:nowISO()};
  ...
  function openTaskModal(p,existing=null) {
    ...
    `,`<button class="button secondary" type="button" id="cancelModalBtn">Cancel</button><button class="button primary" type="button" id="saveTaskBtn">${existing?"Save":"Add task"}</button>`,()=>{
      ...
      $("#saveTaskBtn").onclick=()=>{
        ...
        if(existing){
          Object.assign(existing,{title,section:$("#mtSection").value,priority:$("#mtPriority").value,dueDate:$("#mtDue").value,tags:normalizeTags($("#mtTags").value),notes:$("#mtNotes").value.trim(),waitingFor:$("#mtWaiting").value.trim(),updatedAt:nowISO()});
        }else{
          p.tasks.push({id:uid(),title,section:$("#mtSection").value,priority:$("#mtPriority").value,dueDate:$("#mtDue").value,tags:normalizeTags($("#mtTags").value),notes:$("#mtNotes").value.trim(),waitingFor:$("#mtWaiting").value.trim(),done:false,createdAt:nowISO(),updatedAt:nowISO()});
        }
