...
(() => {
  ...
  function scheduleAutoSync() {
    ...
    autoSyncTimer = setTimeout(() => {
      const dlg = $("#modal");
      // Do not let a background cloud merge replace application objects while
      // the user is actively editing a modal form. Try again after the dialog closes.
      if (dlg?.open) {
        scheduleAutoSync();
        return;
      }
  ...
  function renderProjectTasks(root,p){
    ...
    root.innerHTML=`
      <div class="panel">
        ...
        <div class="quick-task-add">
          <input class="field" id="quickProjectTask" type="text" placeholder="Quick task…">
          <button class="button secondary small" id="quickProjectTaskBtn">Add</button>
        </div>
    ...
    const quickAdd=()=>{
      const input=$("#quickProjectTask");
      const title=input?.value.trim();
      if(!title)return;
      const lp=projectById(p.id);
      if(!lp){toast("Project could not be found");return;}
      const stamp=nowISO();
      lp.tasks.push({
        id:uid(),title,section:lp.sections?.[0]||"General",priority:"Normal",
        dueDate:"",tags:[],notes:"",waitingFor:"",done:false,
        createdAt:stamp,updatedAt:stamp
      });
      lp.updatedAt=stamp;
      persist();
      render();
      setTimeout(()=>$("#quickProjectTask")?.focus(),0);
      toast("Task added");
    };
    $("#quickProjectTaskBtn").onclick=quickAdd;
    $("#quickProjectTask")?.addEventListener("keydown",e=>{
      if(e.key==="Enter"){
        e.preventDefault();
        quickAdd();
      }
  ...
  function renderMeetings(root){
    ...
    root.innerHTML=`
      <div class="page-header"><div><h1 class="page-title">Meetings</h1><p class="page-subtitle">Meeting notes and action items</p></div>
        <button class="button primary" id="newMeetingBtn">＋ New meeting</button></div>
      <div class="panel">
        ${meetings.length?meetings.map(meetingCardHTML).join(""):`<div class="empty">No meeting notes yet.</div>`}
      </div>`;
  ...
  function openMeetingModal(existing=null,presetProjectId=""){
    ...
    const existingActions=(m.actions||[]).map(a=>{
      const projectName = a.projectId
        ? (projectById(a.projectId)?.name || "Project")
        : "General Tasks";
      return `<div class="meeting-existing-action">
        <div class="row-title">${esc(a.text||"")}</div>
        <div class="row-meta"><span>→ ${esc(projectName)}</span></div>
      </div>`;
    }).join("");
    ...
    openModal(existing?"Meeting notes":"New meeting",`
      ...
      <div class="form-row">
        ...
        <div class="field-help">Select all projects discussed in this meeting.</div>
        ${projectsPicker}
      </div>
      ...
      <div class="form-row">
        <div class="field-label-row">
          ...
          <button type="button" class="button secondary small" id="addMeetingActionBtn">＋ Add action item</button>
        </div>
        <div class="field-help">Assign each action item to the specific project it belongs to. Saving the meeting creates the task directly in that project's To-do list.</div>
        <div id="mmActionRows" class="meeting-action-rows"></div>
      </div>

      ${existingActions ? `
        <div class="form-row">
          <label class="field-label">Already created from this meeting</label>
          <div class="meeting-existing-actions">${existingActions}</div>
        </div>` : ""}

      ${existing?`<hr class="sep"><button type="button" class="button danger small" id="deleteMeetingBtn">Delete meeting</button>`:""}
    `,`<button type="button" class="button secondary" id="cancelModalBtn">Cancel</button><button type="button" class="button primary" id="saveMeetingBtn">Save meeting</button>`,()=>{
      ...
      const selectedProjectsNow=()=>$$('#mmProjects input[type="checkbox"]:checked').map(x=>x.value);

      function actionProjectOptions(currentValue=""){
        const ids=selectedProjectsNow();
        let html="";
        if(ids.length>1){
          html += `<option value="__choose__" ${!currentValue||currentValue==="__choose__"?"selected":""}>Choose project…</option>`;
        }
        for(const id of ids){
          const p=projectById(id);
          if(p) html += `<option value="${esc(id)}" ${currentValue===id?"selected":""}>${esc(p.name)}</option>`;
        }
        html += `<option value="" ${currentValue==="" && ids.length<=1?"selected":""}>General Tasks</option>`;
        return html;
      }

      function defaultActionProject(){
        const ids=selectedProjectsNow();
        return ids.length===1 ? ids[0] : (ids.length>1 ? "__choose__" : "");
      }

      function addActionRow(text="",projectId=null){
        const current = projectId===null ? defaultActionProject() : projectId;
        const row=document.createElement("div");
        row.className="meeting-action-row";
        row.innerHTML=`
          <input class="field meeting-action-text" type="text" placeholder="Task / action item" value="${esc(text)}">
          <select class="field meeting-action-project" aria-label="Project for action item">${actionProjectOptions(current)}</select>
          <button type="button" class="row-button meeting-action-remove" aria-label="Remove action item">Remove</button>
        `;
        $("#mmActionRows").appendChild(row);
        applyAutomaticTextDirection(row);
        $(".meeting-action-remove",row).onclick=()=>row.remove();
      }

      // Start with one convenient empty action row.
      addActionRow();
      $("#addMeetingActionBtn").onclick=()=>addActionRow();

      // When meeting projects change, refresh each action's choices.
      $$('#mmProjects input[type="checkbox"]').forEach(ch=>ch.addEventListener("change",()=>{
        $$(".meeting-action-project").forEach(sel=>{
          const previous=sel.value;
          const ids=selectedProjectsNow();
          let next=previous;
          if(previous && previous!=="__choose__" && !ids.includes(previous)){
            next=ids.length===1 ? ids[0] : (ids.length>1 ? "__choose__" : "");
          }
          if(previous==="__choose__" && ids.length===1) next=ids[0];
          if(ids.length===0) next="";
          sel.innerHTML=actionProjectOptions(next);
          sel.value=next;
        });
      }));
      ...
      $("#saveMeetingBtn").onclick=()=>{
        ...
        const projectIds = selectedProjectsNow();
        const newActions = $$(".meeting-action-row").map(row=>({
          text:$(".meeting-action-text",row).value.trim(),
          projectId:$(".meeting-action-project",row).value
        })).filter(a=>a.text);

        if(newActions.some(a=>a.projectId==="__choose__")){
          toast("Choose a project for each action item");
          return;
        }

        const liveMeeting = existing ? (meetingById(existing.id)||existing) : null;
        const linkedActions=liveMeeting ? [...(liveMeeting.actions||[])] : [];
        ...
        for(const action of newActions){
          ...
          if(projectId){
            const p=liveProject(projectId);
            if(!p){
              toast("One selected project could not be found");
              return;
            }
            const task={
              id:uid(),title:text,section:"General",priority:"Normal",dueDate:"",
              tags:[],notes:`From meeting: ${title}`,waitingFor:"",done:false,
            ...
            const task={
              id:uid(),title:text,priority:"Normal",dueDate:"",tags:[],
              notes:`From meeting: ${title}`,waitingFor:"",done:false,
              createdAt:nowISO(),updatedAt:nowISO()
            };
        ...
        const source=liveMeeting||m;
        const data={
          title,
          date:$("#mmDate").value,
          projectIds,
          participants:$("#mmParticipants").value.trim(),
          notes:$("#mmNotes").value.trim(),
        ...
        if(liveMeeting){
          Object.assign(liveMeeting,data);
          delete liveMeeting.projectId;
          delete liveMeeting.summary;
        }else{
          state.meetings.unshift({id:uid(),createdAt:nowISO(),...data});
        }

        persist();closeModal();render();toast("Meeting saved");
      };

      if(existing)$("#deleteMeetingBtn").onclick=()=>{
        if(confirm("Delete this meeting? Linked tasks will remain.")){
          state.meetings=state.meetings.filter(x=>x.id!==existing.id);
  ...
  function renderSettings(root) {
    root.innerHTML=`
      <div class="page-header"><div><h1 class="page-title">Settings & Sync</h1><div class="row-meta">ResearchFlow v0.4.8</div><p class="page-subtitle">Local-first storage with optional OneDrive synchronization</p></div></div>
  ...
  function closeModal(){
    $("#modal").close();
    if(state.settings.syncEnabled && state.settings.autoSync) scheduleAutoSync();
  }
  ...
  function openTaskModal(projectOrId,existingOrId=null) {
    ...
    `,existing
      ...
      const saveTask=(addAnother=false)=>{
        ...
        if(!values.title){toast("Task title is required");return;}

        savingTask=true;
        try{
          const lp=projectById(projectId);
          if(!lp){toast("Project could not be found");closeModal();return;}

          const stamp=nowISO();
          if(existingId){
            const liveTask=lp.tasks.find(x=>x.id===existingId);
            if(!liveTask){toast("Task could not be found");closeModal();return;}
            Object.assign(liveTask,{...values,updatedAt:stamp});
          }else{
            lp.tasks.push({id:uid(),...values,done:false,createdAt:stamp,updatedAt:stamp});
          }
          lp.updatedAt=stamp;
          persist();

          if(addAnother && !existingId){
            // Keep the same dialog open. This avoids repeated HTMLDialogElement
            // close/show cycles and prevents stale modal handlers on installed PWAs.
            $("#mtTitle").value="";
            $("#mtDue").value="";
            $("#mtTags").value="";
            $("#mtWaiting").value="";
            $("#mtNotes").value="";
            applyAutomaticTextDirection($("#mtTitle"));
            applyAutomaticTextDirection($("#mtNotes"));

            // Refresh the page behind the modal from current state.
            render();
            $("#mtTitle")?.focus();
            toast("Task added — enter the next one");
          }else{
            closeModal();
            render();
            toast(existingId?"Task saved":"Task added");
          }
        }finally{
          savingTask=false;
        }
      ...
      $("#mtTitle")?.addEventListener("keydown",e=>{
        if(e.key==="Enter" && !e.shiftKey && !existingId){
          e.preventDefault();
          saveTask(true);
        }
      });
