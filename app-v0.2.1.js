...
(() => {
  ...
  function readableSyncError(e){
    ...
    if(/mysite|OneDrive.*not provisioned|drive.*not found|ResourceNotFound/i.test(raw)){
      return "The signed-in Microsoft identity does not appear to have the OneDrive you use. Click “Reconnect personal OneDrive” and choose your Personal Microsoft account.";
    }
    if(/403|accessDenied|insufficient privileges|Authorization_RequestDenied/i.test(raw)){
      return "Microsoft denied access to the ResearchFlow App Folder. Confirm Files.ReadWrite.AppFolder is a Delegated Microsoft Graph permission, then reconnect and consent again.";
    }
    if(/401|invalid_grant|interaction_required/i.test(raw)){
      return "The Microsoft sign-in session needs to be renewed. Click “Reconnect personal OneDrive”.";
    }
