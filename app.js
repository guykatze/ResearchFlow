...
(() => {
  ...
  function renderSettings(root) {
        ...
        <div>
          <div class="panel">
            ...
            ${state.settings.syncEnabled?`
              <div class="success-box">Connected${state.settings.oneDriveAccount?` as <strong>${esc(state.settings.oneDriveAccount)}</strong>`:""}. ResearchFlow stores one JSON data file in its private OneDrive App Folder.</div>
    ...
    if(state.settings.syncEnabled){
      ...
      $("#pullBtn").addEventListener("click",()=>pullFromOneDrive());
      $("#pushBtn").addEventListener("click",()=>pushToOneDrive());
      $("#disconnectBtn").addEventListener("click",async()=>{
        if(confirm("Disconnect OneDrive? Your local data will stay on this device.")){
          ...
          state.settings.syncEnabled=false;state.settings.oneDriveAccount="";state.settings.lastSyncedAt="";
  ...
  async function connectOneDrive() {
    ...
    let result;
    try{
      result=await app.loginPopup({scopes:graphScopes,prompt:"select_account"});
    }catch(e){
      // Popup is sometimes blocked in standalone iOS PWAs; redirect is the fallback.
      if(String(e?.errorCode||e).includes("popup")) {
        await app.loginRedirect({scopes:graphScopes});
        return;
      }
      throw e;
    }
  ...
  async function getAccessToken() {
    ...
    if(!account){
      const login=await app.loginPopup({scopes:graphScopes,prompt:"select_account"});
      account=login.account;
    }
    try{
      const r=await app.acquireTokenSilent({scopes:graphScopes,account});
      return r.accessToken;
    }catch{
      const r=await app.acquireTokenPopup({scopes:graphScopes,account});
      return r.accessToken;
    }
  }
  ...
  async function graph(path,options={}) {
    ...
    if(!res.ok){
      ...
      const e=new Error(`OneDrive error ${res.status}: ${txt.slice(0,220)}`);
  ...
  async function getRemotePayload() {
    try{
      const metaRes=await graph(`/me/drive/special/approot:/${encodeURIComponent(CLOUD_FILE)}`);
      const meta=await metaRes.json();
      const dl=meta["@microsoft.graph.downloadUrl"];
      if(!dl) throw new Error("OneDrive did not return a download address.");
      const dataRes=await fetch(dl);
      if(!dataRes.ok) throw new Error("Could not download OneDrive data.");
  ...
  async function syncNow({quiet=false}={}) {
    ...
    try{
      ...
      if(remote){
        const merged=mergeCloudData(state,remote);
        state.projects=merged.projects;state.inbox=merged.inbox;state.modifiedAt=merged.modifiedAt;
        persist({touch:false,autosync:false});
      }
      ...
      console.error(e);
      if(!quiet)toast("Sync failed — local data is safe");
    }
  }
