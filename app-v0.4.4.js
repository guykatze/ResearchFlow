...
(() => {
  ...
  function renderSettings(root) {
    root.innerHTML=`
      <div class="page-header"><div><h1 class="page-title">Settings & Sync</h1><div class="row-meta">ResearchFlow v0.4.4</div><p class="page-subtitle">Local-first storage with optional OneDrive synchronization</p></div></div>
  ...
  function openProjectModal(existing=null) {
    ...
    `,`<button class="button secondary" type="button" id="cancelModalBtn">Cancel</button><button class="button primary" type="button" id="saveProjectBtn">${existing?"Save":"Create"}</button>`,()=>{
      ...
      $("#saveProjectBtn").onclick=()=>{
        ...
        if(existing){
          Object.assign(existing,{name,summary:$("#mpSummary").value.trim(),status:$("#mpStatus").value,priority:$("#mpPriority").value,deadline:$("#mpDeadline").value,tags:normalizeTags($("#mpTags").value),updatedAt:nowISO()});
        }else{
